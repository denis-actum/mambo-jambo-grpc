using Grpc.Core;
using Movies.GrpcService;

namespace Movies.GrpcService.Services
{
        ...
        private static readonly List<Movie> _topMovies = new List<Movie>
    {
        ...
        public override Task<TopMoviesResponse> GetTopMovies(TopMoviesRequest request, ServerCallContext context)
        {
            var response = new TopMoviesResponse();
            response.Movies.AddRange(_topMovies.Take(request.Count));
